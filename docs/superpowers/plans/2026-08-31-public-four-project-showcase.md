...
**Architecture:** Keep the existing vinext single-page application and its typed project data. Add the CVMax case to the shared data model, remove the resume-copy feature, and use one server-rendered progressive-enhancement component for motion so the page remains readable without JavaScript and avoids React hydration instability.
...
- [ ] Change the content test to require exactly four project objects and the title `FillResume｜CVMax 网申助手复刻`.
...
- [ ] Add assertions for `42/42 单元测试`, `clean-room`, `不自动提交` and `Chrome E2E`.
- [ ] Run `npm run test:content` and confirm it fails because the fourth project is missing.
- [ ] Extend the project ID union with `resume-autofill`, remove `resumeCopy`, and add the evidence-bounded CVMax project object.
